...
defined('BASEPATH') or exit('No direct script access allowed');
    //====================
    //Função para verificar os parâmetros vindo do FrontEnd
    //====================
        ...
        switch ($tipo){
            ...
            case 'date':
                ...
                if(!preg_match('/^(\d{4})-(\d{2})-(\d{2})$/', $valor, $match)){
                    ...
                    //Tebta criar DataTime no formato y-m-d
        ...
        if ($valor != ''){
            switch ($tipo){
                case 'int':
                    ...
                    break;
                case 'string':
                    //Garante que é um string não vazia após trim
                    if (!is_string($valor) || trim($valor) === ''){
                        return array('codigoHelper' => 5, 'msg' => 'Conteúdo não é um texto');
                    }
                    break;
    ...
    */
    function compararDataHora($valorInicial, $valorFinal, $tipo) {
        ...
        //Valor default de variável $retorno caso não ocorra erro
        return array("codigoHelper" => 0, 'msg' => 'Validação correta.');
    }
